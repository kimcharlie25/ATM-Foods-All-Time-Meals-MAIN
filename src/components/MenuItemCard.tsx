        ...
        <div className="relative h-48 bg-gradient-to-br from-gray-50 to-gray-100">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {item.isOnDiscount && item.discountPrice && (
              <div className="bg-brand-red text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-brand animate-pulse-soft">
                🔥 SALE
              </div>
            )}
            {item.popular && (
              <div className="bg-brand-green text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-green">
                ⭐ POPULAR
              </div>
            )}
          </div>
          ...
          {showDiscount && discountedPrice !== undefined && (
            <div className="absolute bottom-3 right-3 bg-white/95 backdrop-blur-sm text-brand-red text-xs font-bold px-2.5 py-1.5 rounded-full shadow-brand border border-brand-red/20">
            ...
            <div className="flex-shrink-0">
              {!item.available ? (
                ...
                <button
                  ...
                  className="bg-brand-red text-white px-6 py-2.5 rounded-xl hover:bg-brand-red-dark transition-all duration-300 transform hover:scale-105 font-semibold text-sm shadow-brand hover:shadow-brand-lg"
                >
                ...
                <div className="flex items-center space-x-2 bg-brand-green rounded-xl p-1 shadow-green">
                  <button
                    ...
                    className="p-2 hover:bg-brand-green-dark rounded-lg transition-colors duration-200 hover:scale-110"
                  >
          ...
          {item.trackInventory && item.stockQuantity !== null && (
            <div className="mt-3">
              {item.stockQuantity > item.lowStockThreshold ? (
                <div className="flex items-center space-x-2 text-xs text-brand-green bg-green-50 px-3 py-2 rounded-lg border border-brand-green">
                  <span className="font-semibold">✓</span>
                  <span className="font-medium">{item.stockQuantity} in stock</span>
                </div>
              ) : item.stockQuantity > 0 ? (
                <div className="flex items-center space-x-2 text-xs text-yellow-700 bg-yellow-50 px-3 py-2 rounded-lg border border-yellow-400 animate-pulse">
                  <span className="font-semibold">⚠️</span>
                  <span className="font-medium">Only {item.stockQuantity} left!</span>
                </div>
              ) : (
                <div className="flex items-center space-x-2 text-xs text-brand-red bg-red-50 px-3 py-2 rounded-lg border border-brand-red">
                  <span className="font-semibold">✕</span>
            ...
            <div className="p-6">
                      ...
                      <label
                        ...
                        className={`flex items-center justify-between p-4 border-2 rounded-xl cursor-pointer transition-all duration-300 ${
                          selectedVariation?.id === variation.id
                            ? 'border-brand-red bg-red-50 shadow-brand'
                            : 'border-gray-200 hover:border-brand-red/30 hover:bg-gray-50'
                        }`}
                      >
                              ...
                              {selectedAddOns.find(a => a.id === addOn.id) ? (
                                <div className="flex items-center space-x-2 bg-brand-green rounded-xl p-1 shadow-green">
                                  <button
                                    ...
                                    className="p-1.5 hover:bg-brand-green-dark rounded-lg transition-colors duration-200"
                                  >
                                    <Minus className="h-3 w-3 text-white" />
                                  </button>
                                  <span className="font-semibold text-white min-w-[24px] text-center text-sm">
                                ...
                                <button
                                  ...
                                  className="flex items-center space-x-1 px-4 py-2 bg-brand-red text-white rounded-xl hover:bg-brand-red-dark transition-all duration-300 text-sm font-semibold shadow-brand"
                                >
              ...
              <div className="border-t border-gray-200 pt-4 mb-6">
                <div className="flex items-center justify-between text-2xl font-bold text-black">
                  <span>Total:</span>
                  <span className="text-brand-red">₱{calculatePrice().toFixed(2)}</span>
                </div>
              </div>

              <button
                onClick={handleCustomizedAddToCart}
                className="w-full bg-brand-red text-white py-4 rounded-xl hover:bg-brand-red-dark transition-all duration-300 font-bold flex items-center justify-center space-x-2 shadow-brand-lg hover:shadow-brand-lg transform hover:scale-105"
              >
